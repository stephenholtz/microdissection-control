classdef LaserIO
    ...
    methods
        ...
        function [dataIn,dataOut,obj] = runDissection(obj, check_conf)
            ...
            if check_conf
                ready = obj.checkConfPreDissection();
                if ~ready
                    warning('Aborting')
                    return
                end
            ...
            % 10 samples @ 10kHz works here, probably should be a constant
            pulse = 1*[0*ones(1,(obj.session.Rate * 1/obj.pulseFrequency)-100) ones(1,10)  0.*ones(1,90)];
